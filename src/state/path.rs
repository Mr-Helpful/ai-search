...
pub struct PathState<S: State, C: SearchCost<S>> {
  state: S,
  actn_cost: C,
  path_cost: C::Cost,
}

impl<S: State, C: SearchCost<S>> PathState<S, C> {
...
impl<S: State, C: SearchCost<S> + Clone> State for PathState<S, C> {
  type Error = S::Error;
  ...
  fn result(&self, action: &Self::Action) -> Result<Self, Self::ResultError> {
    self.state.result(action).map(|state| PathState {
      state,
